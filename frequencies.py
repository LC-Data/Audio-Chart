...
file = "GRAND"	#File name is split up in to 3 pieces like this in case we want to create multiple files, so we can increment the number
fileNum = 1
ext = ".wav"

audioData = [];		#this is an empty list, that will be populated with y-values from the resultant sine wave (I think)
...
for x in range(0,10):
	...
	frequency = frequency + 15;		#increment the frequency to change the sound on the next iteration
